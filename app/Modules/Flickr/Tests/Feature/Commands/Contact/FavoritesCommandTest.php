...
class FavoritesCommandTest extends TestCase
{
    public function testHandle()
    {
        ...
        $this->assertEquals(1, $contact->tasks()->where('task', TaskService::TASK_CONTACT_FAVORITES)->count());
        ...
        Queue::assertPushed(ContactFavoritesJob::class, function ($job) use ($contact) {
            return $job->integration->isState(CompletedState::class)
                && $job->task->model->is($contact);
        });
    }
}
